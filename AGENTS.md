...
- **A practice run ends on `wonrace`, not `finish`.** `CGameOver::Enter` plays `finish` for
  anything that is not a cup race, and there are no cups here, so faithfully that would be every
  practice run. `finish` is the stand-up all three clips share — a second and more crouched with
  the head down — and then it simply stops on a flat stance, which at the end of every run reads
  as a sulk. `wonrace` is the same stand-up with a lean back and a hop on the end, and it matches
  what the player already hears: ETR plays the win sting for any practice run that was not
  aborted. A field race and a ghost race still win or lose on the result; `finish` survives only
  as the fallback for a rig without the clip. `RaceOutcome.clip`.
